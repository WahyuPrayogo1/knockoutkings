...
export default function HeroWorkoutForWomen() {
  ...
  return (
    ...
    <section className="w-full bg-[#f5f5f5] dark:bg-[#121212] transition duration-500" ref={ref}>
      ...
      ></div>
      <main className="flex flex-col justify-between p-64 items-center lg:p-64 md:py-96 sm:py-80 sm:px-6 py-72 px-6 z-10 w-full h-full relative tersembunyi-fade">
        <div id="hero-left">
          <div
            className="text-white lg:text-7xl md:text-5xl text-4xl flex flex-row lg:gap-4 md:gap-3 sm:gap-2 gap-1"
            style={{ fontFamily: 'Bebas Neue' }}
          >
            workout for <h3 style={{ color: 'red' }}>Women</h3>
          </div>
        </div>
        <div id="hero-right">
          <h3 className="text-white text-center uppercase lg:text-base md:text-base sm:text-sm text-[10px]" style={{ fontFamily: 'Roboto' }}>
          SCROLL DOWN TO ARRAnGE your workout schedule for Women
          </h3>
        </div>
      </main>
      </div>
