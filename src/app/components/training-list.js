...
const TrainingListandUserTrainingList = () => {
    ...
    return (
        <section id="gallery" className="w-full lg:px-12 px-6 py-12  -mt-2.5 dark:bg-[#121212] bg-[#f5f5f5] transition duration-500">

        <div className="dark:text-white text-black uppercase gap-2 text-center lg:text-6xl text-4xl md:py-8 py-4 transition duration-500" style={{fontFamily: 'Bebas Neue'}}>choose your martial arts training schedule</div>
        <p className="text-center dark:text-white text-black font-medium lg:text-md md:text-sm text-xs leading-relaxed py-4 transition duration-500 max-w-7xl mx-auto" style={{fontFamily: 'Roboto'}}>Welcome to Knocknout King's Training Section. We are committed to providing high-quality training programs that empower individuals and organizations to achieve their goals. Below, you will find information about our training offerings, schedules, and how to enroll.</p>
            ...
            {TrainingList.map((item,index) => (
                ...
                <div key={index} className="relative overflow-hidden shadow-black shadow-lg w-full md:h-[350px] h-[250px] group">
                    ...
                    <div className="bottom-0 left-0 absolute z-20 px-3 py-3">
                        ...
                        <Link href={item.href}>
                            <button className="text-white hover:text-black md:px-6 md:py-2 px-3 py-1 md:text-base text-sm opacity-0 group-hover:opacity-100 bg-red-600 hover:bg-white transition duration-500 uppercase">{item.button}</button>
                        </Link>
                    </div>
